        ...
        @can('delete', $outlet)
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <p>{{ $outlet->address }}</p>
                    <label class="control-label text-primary">تلفن ثابت</label>
                    <p>{{ $outlet->telefon }}</p>
                    <label class="control-label text-primary">تلفن همراه</label>
                    <p>{{ $outlet->phone }}</p>
            ...
            <form method="POST" action="{{ route('outlets.update', $outlet) }}" accept-charset="UTF-8">
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="telefon" class="control-label">تلفن ثابت</label>
                                <input id="telefon" type="text" class="form-control{{ $errors->has('telefon') ? ' is-invalid' : '' }}" name="telefon" value="{{ old('telefon',  $outlet->telefon) }}" required>
                                {!! $errors->first('telefon', '<span class="invalid-feedback" role="alert">:message</span>') !!}
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="phone" class="control-label">شماره همراه</label>
                                <input id="phone" type="text" class="form-control{{ $errors->has('phone') ? ' is-invalid' : '' }}" name="phone" value="{{ old('phone',  $outlet->phone) }}" required>
                    ...
                    <div class="form-group">
                        <label for="desc" class="control-label">توضیحات</label>
                        <textarea id="desc" class="form-control{{ $errors->has('desc') ? ' is-invalid' : '' }}" name="desc" rows="4">{{ old('desc', $outlet->desc) }}</textarea>
                        {!! $errors->first('desc', '<span class="invalid-feedback" role="alert">:message</span>') !!}
                    </div>
